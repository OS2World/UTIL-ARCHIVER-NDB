...
//    fprintf(stdout, "MD5_Update: start_time = %llu\n", start_time);
    fflush(stdout);
#endif
  ...
  memcpy(&pCtx->buffer[lIndex],
    ...
    if (ndb_getDebugLevel() >= 1)
    {
		...
		snprintf(s, 199, "MD5_Update: count %lu, average %lu, max %lu, sum %lu\n",
						 dbg_count, (dbg_timall / dbg_count), dbg_timmax, dbg_timall);
        ndb_util_set_performancetest(TEST_PERFORMANCE_MD5, s);
    }
#endif
}
